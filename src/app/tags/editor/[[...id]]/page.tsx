    ...
    <main className="pb-24 pt-6 px-20">
      <Breadcrumb
        items={[
          ...
          { label: "افزودن/ویرایش تگ", href: "/article/new-tag" },
        ]}
      />
      <div className="mt-5">
        <TagForm />
...
function TagForm() {
  ...
  // اگر مسیرت /article/new-tag/[id] باشد:
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) throw new Error("خطا در دریافت تگ");
        ...
        if (active) setError(e?.message || "خطا در دریافت تگ");
      } finally {
