...
export default async function UsersListPage({
  ...
  const createdFrom =
    typeof searchParams.createdFrom === "string"
      ? searchParams.createdFrom
      : undefined;
  const createdTo =
    typeof searchParams.createdTo === "string"
      ? searchParams.createdTo
      : undefined;
  const withDeleted = (searchParams.withDeleted ?? "1") as string | undefined;
  const deletedOnly =
    typeof searchParams.deletedOnly === "string"
      ? searchParams.deletedOnly
      : undefined;
    ...
    <main className="pb-24 pt-6">
      <div className="mx-auto w-full max-w-7xl 2xl:max-w-[110rem]">
        ...
        <div className="mt-4 text-sm text-gray-500">
          {list.total.toLocaleString("fa-IR")} کاربر • صفحه {list.page} از{" "}
          {list.pages}
        </div>
