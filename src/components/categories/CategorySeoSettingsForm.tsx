      ...
      <div className="hidden md:flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <input
            ...
            disabled={!categoryId}
          />
          <label htmlFor="seo-use-auto" className="text-sm text-black">استفاده از مقادیر خودکار</label>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="mt-4 md:mt-6">
        <div className="flex md:hidden items-center gap-2 mb-4">
          <input
            ...
            disabled={!categoryId}
          />
          <label htmlFor="seo-use-auto-m" className="text-sm text-black">استفاده از مقادیر خودکار</label>
        </div>
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 sm:gap-6">
          <div className="xl:col-span-7 space-y-6">
            <fieldset className="space-y-4">
              ...
              <LabeledSelect label="Robots" value={(form.robots ?? "") as any} onChange={handleChange("robots")} options={[{ label: "پیش‌فرض", value: "" }, { label: "index,follow", value: "index,follow" }, { label: "noindex,follow", value: "noindex,follow" }, { label: "index,nofollow", value: "index,nofollow" }, { label: "noindex,nofollow", value: "noindex,nofollow" }]} disabled={!categoryId || isFieldsDisabled} />
            </fieldset>
            <fieldset className="space-y-4">
          ...
          <div className="xl:col-span-5 space-y-6">
            <fieldset className="space-y-4">
              ...
              <LabeledInput label="OG Image URL" placeholder="https://..." value={form.ogImageUrl ?? ""} onChange={handleChange("ogImageUrl")} disabled={!categoryId || isFieldsDisabled} />
              <div className="rounded-xl border border-gray-200 overflow-hidden h-[180px] sm:h-[200px] md:h-[220px] flex items-center justify-center bg-gray-50">
                {form.ogImageUrl ? <img src={form.ogImageUrl} alt="OG preview" className="w-full h-full object-cover" /> : <div className="text-xs text-gray-400">پیش‌نمایش تصویر OG</div>}
              </div>
            </fieldset>
            <fieldset className="space-y-3">
              <legend className="font-medium text-black">پیش‌نمایش SERP</legend>
              <div className="rounded-xl border border-gray-200 p-4">
                <div className="text-[#1a0dab] text-base sm:text-lg leading-6 truncate">{form.seoTitle || "عنوان دسته"}</div>
                <div className="text-[#006621] text-[12px] mt-1 truncate">{form.canonicalUrl || "https://example.com/category/slug"}</div>
                <div className="text-[#545454] text-[13px] mt-1 line-clamp-2">{form.seoDescription || "توضیحات متا."}</div>
              </div>
            </fieldset>
          </div>
        </div>
