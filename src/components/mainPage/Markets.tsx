  ...
  return (
    <div className="p-4 sm:p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg sm:text-xl font-bold text-[#1C2121]">{title}</h3>
      </div>
...
function PairRow({ item }: { item: Pair }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 px-3 py-3 border-b last:border-b-0">
      <div className="flex items-center gap-3">
        <Image src={item.icon} alt={item.name} width={48} height={34} />
        <div className="text-left">
          <div className="text-sm font-semibold text-[#2E3232]">{item.name}</div>
          <div className="text-xs text-[#666969]">{item.market}</div>
        </div>
      </div>

      <div className="text-left min-w-[90px]">
        <div className="text-sm font-semibold text-[#2E3232]">${item.price}</div>
