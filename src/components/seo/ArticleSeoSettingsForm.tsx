...
export default function SeoSettingsForm({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const qs = new URLSearchParams({ entityType, entityId, locale }).toString();
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.text().catch(() => "")) || "خطا در ذخیره تنظیمات سئو");
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.text().catch(() => "")) || "حذف تنظیمات ناموفق بود");
      setExists(false);
      setForm({
        useAuto: true,
        seoTitle: null,
  ...
  return (
    <div dir="rtl" className="bg-skin-card rounded-2xl shadow-sm border border-skin-border p-4 sm:p-6 md:p-8">
      {!entityId && (
        <div className="mb-4 rounded border p-3 border-amber-300 bg-amber-50 text-amber-800 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
          برای تنظیم سئو، ابتدا مقاله را ذخیره کنید تا شناسه (ID) داشته باشد.
        </div>
      )}
      {error && (
        <div className="mb-4 rounded border p-3 border-red-300 bg-red-50 text-red-700 dark:border-red-500/40 dark:bg-red-500/10 dark:text-red-300">
          {error}
        </div>
      )}
      {loading ? (
        <div className="text-skin-muted">در حال بارگذاری تنظیمات سئو…</div>
      ) : (
        <>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-2">
              <input
                ...
                disabled={!entityId}
              />
              <label htmlFor="seo-use-auto" className="text-sm text-skin-base">استفاده از مقادیر خودکار (پیشنهادی)</label>
            </div>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-2 w-full sm:w-auto">
              <button
                type="button"
                onClick={onDelete}
                className="px-4 py-2 rounded-lg border border-skin-border text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 disabled:opacity-50 w-full sm:w-auto"
                disabled={!entityId || deleting || !exists}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6 mt-6">
            <div className="md:col-span-7 space-y-4 sm:space-y-6">
              <fieldset className="space-y-4">
                <legend className="font-medium text-skin-heading">SEO Basics</legend>
                <LabeledInput
                  label="SEO Title"
                  ...
                  max={60}
                  disabled={!entityId || isFieldsDisabled}
                />
                <LabeledTextarea
              ...
              <fieldset className="space-y-4">
                <legend className="font-medium text-skin-heading">Twitter</legend>
                <LabeledSelect
                  label="Twitter Card"
              ...
              <fieldset className="space-y-4">
                <legend className="font-medium text-skin-heading">Article Meta</legend>
                <LabeledInput
                  label="Author Name"
            ...
            <div className="md:col-span-5 space-y-4 sm:space-y-6">
              <fieldset className="space-y-4">
                <legend className="font-medium text-skin-heading">Open Graph</legend>
                <LabeledInput
                  label="OG Title"
                ...
                <div className="rounded-xl border border-skin-border overflow-hidden h-40 sm:h-48 md:h-44 lg:h-[160px] flex items-center justify-center bg-skin-bg">
                  {form.ogImageUrl ? (
                    ...
                    <div className="text-xs text-skin-muted">پیش‌نمایش تصویر OG</div>
                  )}
                </div>
              </fieldset>

              <fieldset className="space-y-3">
                <legend className="font-medium text-skin-heading">پیش‌نمایش SERP</legend>
                <div className="rounded-xl border border-skin-border p-3 sm:p-4 bg-skin-bg">
                  <div className="text-skin-heading text-[16px] sm:text-[18px] leading-6 truncate">
                    {form.seoTitle || "عنوان صفحه (نمونه)"}
                  </div>
                  <div className="text-skin-accent text-[11px] sm:text-[12px] mt-1 truncate">
                    {form.canonicalUrl || "https://example.com/article/slug"}
                  </div>
                  <div className="text-skin-muted text-[12px] sm:text-[13px] mt-1 line-clamp-2">
                    {form.seoDescription || "توضیحات متا حداکثر حدود ۱۵۰–۱۶۰ کاراکتر، برای جذب کلیک بهتر."}
                  </div>
                </div>
              </fieldset>
            </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <label className="block text-sm text-skin-muted mb-2">{label}</label>
        {typeof value === "string" && max ? <CharCounter value={value} max={max} /> : null}
      </div>
      <input
        type="text"
        className="w-full rounded-lg border border-skin-border bg-skin-bg text-skin-base px-3 py-2 focus:outline-none focus:ring-2 focus:ring-skin-border disabled:opacity-60"
...
function LabeledTextarea({
  ...
  return (
    <div>
      ...
      <textarea
        className="w-full min-h-24 sm:min-h-[100px] rounded-lg border border-skin-border bg-skin-bg text-skin-base px-3 py-2 focus:outline-none focus:ring-2 focus:ring-skin-border disabled:opacity-60"
        placeholder={placeholder}
        value={value || ""}
        onChange={onChange}
        maxLength={max}
