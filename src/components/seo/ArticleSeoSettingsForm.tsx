...
export default function SeoSettingsForm({
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          (await res.text().catch(() => "")) || "خطا در ذخیره تنظیمات سئو"
        );
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          (await res.text().catch(() => "")) || "حذف تنظیمات ناموفق بود"
        );
      setExists(false);
      setForm((f) => ({
        ...f,
  ...
  return (
    <div
      dir="rtl"
      className="bg-white rounded-2xl shadow-sm border p-4 sm:p-6 md:p-8"
    >
      {!entityId && (
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-2 w-full sm:w-auto">
              <button
                type="button"
                onClick={onDelete}
                className="px-4 py-2 rounded-lg border text-red-600 hover:bg-red-50 disabled:opacity-50 w-full sm:w-auto"
            ...
            <div className="md:col-span-5 space-y-4 sm:space-y-6">
              ...
              <fieldset className="space-y-3">
                ...
                <div className="rounded-xl border border-gray-200 p-3 sm:p-4">
                  <div className="text-[#1a0dab] text-[16px] sm:text-[18px] leading-6 truncate">
                    {form.seoTitle || 'عنوان صفحه (نمونه)'}
                  </div>
                  <div className="text-[#006621] text-[11px] sm:text-[12px] mt-1 truncate">
                    {form.canonicalUrl || 'https://example.com/article/slug'}
                  </div>
                  <div className="text-[#545454] text-[12px] sm:text-[13px] mt-1 line-clamp-2">
...
function LabeledTextarea({
  ...
  return (
    <div>
      ...
      <textarea
        className="w-full min-h-24 sm:min-h-[100px] text-black rounded-lg border border-gray-200 bg-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-300 disabled:opacity-60"
        placeholder={placeholder}
        value={value || ''}
        onChange={onChange}
        maxLength={max}
